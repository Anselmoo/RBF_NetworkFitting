...
def report(dstream):
    ...
    print("Sigma of Gaussian -> {:2.5}".format(np.unique(val['params'][0])[
        0]))
    print(
        "Sigma of Lorentzian -> {:2.5}".format(np.unique(val['params'][1])[
            0]))
...
def cmd():
    ...
    parser = argparse.ArgumentParser(
        description='Process Command Line Interface for the genetic optimized '
        'fitting of'
        'Radial Basis Function Networks')

    parser.add_argument("infile", help="Input data file")
    parser.add_argument("-p", "--para", nargs=3, default=[10, 10, 50],
                        type=int,
                        help="Number of items, evolutions, iterations ("
                             "default: 10, 10, 50)")
    parser.add_argument("-hl", "--hidden", nargs=2, default=[10, 50],
                        type=int,
                        help="Range of hidden shapes (default: 10 to 50)")
    parser.add_argument("-mt", "--mutate", nargs=2, default=[0.1, 0.1],
                        type=float,
    ...
    parser.add_argument("-m", "--method", nargs='+',
                        default=['gaus', 'lorz', 'psed'], type=str,
                        help="Range of methods (default: all three ['gaus', "
                             "'lorz', 'psed'] ")
    ...
    gens = GeneticFitter.GeneticOpt(x, y, items=args.para[0],
                                    epochs=args.para[1],
                                    max_iter=args.para[2],
                                    hidden_ranges=args.hidden,
                                    method=args.method, plot=args.plot)
    ypred = gens.evolver(mutate=args.mutate, incest=args.incest)

    np.savetxt(args.infile.rsplit('.', 1)[0] + '.export',
               np.array([x, y, ypred]).T, delimiter='\t',
               header='X\ty\typred')
