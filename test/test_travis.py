...
def generate_genetic(grid):
	...
	# Just check if everything is running and connected, but no extended test for travis-CI
	gens = GeneticFitter.GeneticOpt(x, y, items=1, epochs=1, max_iter=1, conv_min=10e-9, hidden_ranges=[5, 45],
	                                method=['gaus'])
	y_pred = gens.evolver()
